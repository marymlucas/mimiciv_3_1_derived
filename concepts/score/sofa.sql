...
WITH co AS (
    SELECT ih.stay_id, ie.hadm_id
        ...
        , ih.endtime
    FROM `mymimiciv.mimiciv_3_1_derived.icustay_hourly` ih
...
, pafi AS (
    ...
    INNER JOIN `mymimiciv.mimiciv_3_1_derived.bg` bg
...
, vs AS (
    ...
    FROM co
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.vitalsign` vs
...
, gcs AS (
    ...
    FROM co
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.gcs` gcs
...
, bili AS (
    ...
    FROM co
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.enzyme` enz
...
, cr AS (
    ...
    FROM co
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.chemistry` chem
...
, plt AS (
    ...
    FROM co
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.complete_blood_count` cbc
...
, uo AS (
    SELECT co.stay_id, co.hr
        ...
        , MAX(
            ...
            END) AS uo_24hr
    FROM co
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.urine_output_rate` uo
