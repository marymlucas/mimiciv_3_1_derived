...
WITH pa AS (
    ...
    FROM `mymimiciv.mimiciv_3_1_derived.bg` bg
...
, aa AS (
    ...
    INNER JOIN `physionet-data.mimiciv_3_1_icu.icustays` ie
        ON bg.hadm_id = ie.hadm_id
            AND bg.charttime >= ie.intime AND bg.charttime < ie.outtime
    INNER JOIN `mymimiciv.mimiciv_3_1_derived.ventilation` vd
        ON ie.stay_id = vd.stay_id
            AND bg.charttime >= vd.starttime
            AND bg.charttime <= vd.endtime
            AND vd.ventilation_status = 'InvasiveVent'
...
, arf AS (
    ...
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.first_day_lab` labs
...
, vent AS (
    ...
    FROM `physionet-data.mimiciv_3_1_icu.icustays` ie
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.ventilation` v
...
, cohort AS (
    ...
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.first_day_vitalsign` vital
