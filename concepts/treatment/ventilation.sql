...
WITH tm AS (
    ...
    FROM `mymimiciv.mimiciv_3_1_derived.ventilator_setting`
    UNION DISTINCT
    SELECT stay_id, charttime
    FROM `mymimiciv.mimiciv_3_1_derived.oxygen_delivery`
)
...
, vs AS (
    ...
    FROM tm
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.ventilator_setting` vs
        ON tm.stay_id = vs.stay_id
            AND tm.charttime = vs.charttime
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.oxygen_delivery` od
