WITH cbc AS (
    ...
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.complete_blood_count` le
...
, chem AS (
    ...
    FROM `physionet-data.mimiciv_3_1_icu.icustays` ie
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.chemistry` le
        ON le.subject_id = ie.subject_id
            AND le.charttime >= DATETIME_SUB(ie.intime, INTERVAL '6' HOUR)
            AND le.charttime <= DATETIME_ADD(ie.intime, INTERVAL '1' DAY)
    GROUP BY ie.stay_id
...
, diff AS (
    ...
    FROM `physionet-data.mimiciv_3_1_icu.icustays` ie
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.blood_differential` le
        ON le.subject_id = ie.subject_id
            AND le.charttime >= DATETIME_SUB(ie.intime, INTERVAL '6' HOUR)
            AND le.charttime <= DATETIME_ADD(ie.intime, INTERVAL '1' DAY)
    GROUP BY ie.stay_id
...
, coag AS (
    ...
    FROM `physionet-data.mimiciv_3_1_icu.icustays` ie
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.coagulation` le
        ON le.subject_id = ie.subject_id
            AND le.charttime >= DATETIME_SUB(ie.intime, INTERVAL '6' HOUR)
            AND le.charttime <= DATETIME_ADD(ie.intime, INTERVAL '1' DAY)
    GROUP BY ie.stay_id
...
, enz AS (
    ...
    FROM `physionet-data.mimiciv_3_1_icu.icustays` ie
    LEFT JOIN `mymimiciv.mimiciv_3_1_derived.enzyme` le
        ON le.subject_id = ie.subject_id
            AND le.charttime >= DATETIME_SUB(ie.intime, INTERVAL '6' HOUR)
            AND le.charttime <= DATETIME_ADD(ie.intime, INTERVAL '1' DAY)
    GROUP BY ie.stay_id
